...
52.83% of the males in this dataset folded their arms with the left on top, while 57.66% of the females folded their arms with their left arm on top.
...
z\* = 1.96 (because 95% confidence interval)

se(p1-p2) = sqrt( p1(1-p1)/n1 + p2(1-p2)/n2 )

p1 = proportion of left folding males

p2 = proportion of left folding females

n1 = number of males in sample

n2 = number of females in sample

Confidence Interval: (-0.093, 0.1897)

D.  

I am 95% confident that the true difference in proportions of males who fold their arms with their left arm on top vs females is between -0.093 and 0.1897 (proportion of females is between between -9.3 and +18.97 of that of males).

E.  

The standard error (0.067) represents the standard deviation of the sampling distribution model, the model that shows the distribution of the difference in proportions of hypothetically unlimited such procedures where we randomly get 106 males and 111 females and ask them to fold their arms and see the difference in proportions.

F.  

Sampling distribution is the model that shows what the distribution of the difference of proportions of hypothetically unlimited of these procedures would look like. The proportion of males and females who put their left arm on top is slightly different from sample to sample, everything else stays the same including the sample size, etc.

G.  

This is an aggregation of small nudges, as it is the collective effect of hundreds of males or females folding their arms, making using the normal distribution appropriate.
...
left_males = sum(df$Sex == "Male" & df$LonR_fold == 0)
left_females = sum(df$Sex == "Female" & df$LonR_fold == 0)
...
B. The histograms show how the distribution of age for GOTV call receivers and non receivers have much different distributions. The table afterwards show how the percentage of people who are registered with a major political party varies between people who got the call and didn't. The table afterwards shows the big difference in called percentage for people who voted in 1996 and didn't. The table afterwards shows the massive difference in percentage of people who voted in 1998 after voting in 1996 and the percentage of people who voted in 1998 after not voting in 1996. Finally, the large sample confidence interval at the end shows that the true difference in proportions between 1998 voters who voted in 1996 and didn't is between 39.32% and 42.75%.
